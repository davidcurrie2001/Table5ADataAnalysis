...
MyPlot(to_plot=data_to_plot,title="Overall mean for all MS")
...
for(MSname in unique(MeanByMS$MS)){
  ...
  MyPlot(to_plot=data_to_plot,title=MSname, include_legend = TRUE)
}
...
kable(round(MeanByMS[,c("SamplingDesign","NonResponse","DataCapture","DataStorage","AccuracyBias","EditImpute")],digits=2),caption = "Mean by Member State")
...
for(regionName in unique(MeanByRegion$Region)){
  ...
  MyPlot(to_plot=data_to_plot,title=regionName, include_legend = TRUE)
}
...
kable(round(MeanByRegion[,c("SamplingDesign","NonResponse","DataCapture","DataStorage","AccuracyBias","EditImpute")],digits=2),caption = "Regional Summary")
...
1) Rows which include values other than "1","2","3" or "4" for their indicators.
